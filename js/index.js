...
$(function () {
  ...
  function setVideo(){
    ...
    /* main video */
    var mainW = $('.main_video_wrapper').outerWidth();
    var mainH = $('.main_video_wrapper').outerHeight();
    var mainRation = getMainRatio(mainW, mainH, baseW, baseH);
    $('.main_video').css({
			'width':parseInt(baseW * mainRation),
			'height':parseInt(baseH * mainRation),
			'margin-top':-(parseInt(baseH * mainRation) - mainH) / 2,
			'margin-left':-(parseInt(baseW * mainRation) - mainW) / 2
		});
    
    /* about video */
    var videoW = $('.video_wrapper').outerWidth();
    var videoH = $('.video_wrapper').outerHeight();
    var videoRation = getAboutRatio(videoW, videoH, baseW, baseH);
    $('.video_wrapper .video').css({
			'width':parseInt(baseW * videoRation),
			'height':parseInt(baseH * videoRation),
			'margin-top':-(parseInt(baseH * videoRation) - videoH) / 2,
			'margin-left':-(parseInt(baseW * videoRation) - videoW) / 2
		});
    
    /* recruit video */
    var recruitW = $('.recruit_video_wrapper').outerWidth();
    var recruitH = $('.recruit_video_wrapper').outerHeight();
    var recruitRation = getRecruitRatio(recruitW, recruitH, baseW, baseH);
    $('.recruit_video_wrapper .video').css({
			'width':parseInt(baseW * recruitRation),
			'height':parseInt(baseH * recruitRation),
			'margin-top':-(parseInt(baseH * recruitRation) - recruitH) / 2,
			'margin-left':-(parseInt(baseW * recruitRation) - recruitW) / 2
		});
  }
  /* main */
